...
export class AlandaProjectAndProcessesComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subprocessSelect$
      .pipe(
        ...
        exhaustMap((data) =>
          this.papService.startSubprocess(data).pipe(
    ...
    if (data.type === TreeNodeDataType.PROJECT) {
      ...
      this.getProjectWithProcessesAndTasks(data.project)
        ...
        .subscribe((nodes) => {
          node.children = nodes;
          node.children.push(
            this.papService.getStartProcessDropdownAsTreeNode(),
          );
  ...
  cancelProject(): void {
    this.loading = true;
    this.projectService
      .stopProject(this.dialogData.projectGuid, this.dialogData.reason)
      .pipe(finalize(() => (this.loading = false)))
      .subscribe(() => {
        this.changed.emit();
        this.loadProjectAndProcesses();
  ...
  cancelProcess() {
    ...
    this.projectService
      .stopProjectProcess(
        this.dialogData.projectGuid,
        this.dialogData.processGuid,
  ...
  private loadActionConfig(nodes: TreeNode[]): void {
    for (const node of nodes) {
      if (node.data.project) {
        ...
        node.data.process &&
        node.data.type !== TreeNodeDataType.STARTPROCESS
      ) {
        this.actionsConfig[node.data.uuid] = {
          'CONFIGURE-SUBPROCESS': { display: true },
        };
        if (node.data.process.status === ProjectState.NEW) {
          this.actionsConfig[node.data.uuid]['START-SUBPROCESS'] = {
            display: true,
          };
          this.actionsConfig[node.data.uuid]['REMOVE-SUBPROCESS'] = {
            display: true,
          };
        } else {
          this.actionsConfig[node.data.uuid]['CANCEL-PROCESS'] = {
            display: true,
          };
        }
      }
