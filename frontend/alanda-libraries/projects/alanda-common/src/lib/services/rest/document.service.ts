...
import { ExtendedTreeNode } from '../../components/attachments/shared/tree-node.model';
import { SimpleDocument } from '../../components/attachments/shared/simple-document.model';
...
export class DocumentServiceNg extends ExceptionHandlingService {
  ...
  deleteFile(objectType: string, objectId: number, folderId: number, fileId: number, mappings?: string): Observable<any> {
    ...
    return this.http.delete<any>(`${this.documentEndpointUrl}/refObject/${objectType}/${objectId}/guid/${folderId}/${fileId}`, { params: params })
    .pipe(catchError(this.handleError('deleteFile')));
  }

  getDownloadUrlForVersion(objectType: string, objectId: number, folderId: number, fileId: number, inline: boolean, mappings: string, version: number): string {
    let params = "?";
    if(inline){
  ...
  getDownloadUrlByName(objectType: string, objectId: number, folderName: string, fileId: number, inline?: boolean, mappings?: string): string {
    let params = "?";
    if(inline){
      params += `inline=${inline}&`;
    }
