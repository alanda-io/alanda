...
import { ProjectAndProcessesService, TreeNodeData } from './project-and-processes.service';
...
export class AlandaProjectAndProcessesComponent implements OnInit {
  ...
  onSubprocessSelected(rowData: TreeNodeData, selection: {processName: string, processDefinitionKey: string}) {
    ...
    this.projectService.saveProjectProcess(rowData.relatedProject.guid, process).subscribe(res => {
      ...
      from(this.flattenProjects(projectTree)).pipe(
        concatMap(flattenProject => {
          return this.getProjectWithProcessesAndTasks(flattenProject).pipe(
            map(project => {
              return this.projectAndProcessesService.mapProjectToTreeNode(project, this.project);
            })
          );
        }),
        toArray(),
  ...
  private updateTreeStructure(projects: TreeNode[]): TreeNode[] {
    ...
    const currentProject = projects.filter(node => node.data.type === 'project');
    /* .map(filteredNode => this.projectAndProcessesService
      .mapNewProcessToTreeNode(filteredNode, null)); */
  ...
  private flattenProjects(project: AlandaProject): AlandaProject[] {
    ...
    project.parents?.forEach(parent => {
      parent.childrenIds ? parent.childrenIds.push(project.guid) : parent.childrenIds = [project.guid];
      projects.push(parent);
    });
    projects.push(project);
    project.children?.forEach(child => {
      child.parentIds ? child.parentIds.push(project.guid) : child.parentIds = [project.guid];
      projects.push(child);
    });
    return projects;
  }
